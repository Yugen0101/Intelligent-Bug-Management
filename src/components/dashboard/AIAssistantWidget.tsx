...
import { Send, Bug, Mic, Maximize2, User } from 'lucide-react'
                ...
                {messages.length === 0 ? (
                    <div className="py-6 flex flex-col items-center text-center">
                        ...
                        <div className="relative mb-5 mt-2">
                            <div className="w-16 h-16 rounded-full bg-blue-600 shadow-[0_0_40px_rgba(37,99,235,0.35)] animate-pulse" />
                            <div className="absolute inset-0 w-full h-full rounded-full bg-gradient-to-tr from-blue-800 via-blue-500 to-blue-300 opacity-80" />
                        ...
                        <div className="flex flex-wrap justify-center gap-2">
                            {quickPills.map((pill) => (
                                <button
                                    ...
                                    className="px-3 py-1.5 bg-blue-50 text-blue-600 text-[10px] font-bold rounded-full hover:bg-blue-100 transition-colors border border-blue-100"
                                >
                                    {pill}
                                </button>
                            ))}
                    ...
                    <>
                        {messages.map((msg, i) => (
                            <div key={i} className={cn("flex gap-3", msg.role === 'user' ? "flex-row-reverse" : "flex-row")}>
                                <div className={cn(
                                    ...
                                    {msg.role === 'ai' ? <Bug className="w-3.5 h-3.5 text-blue-600" /> : <User className="w-3.5 h-3.5 text-gray-600" />}
                                </div>
                                <div className={cn(
                                    "p-3 rounded-2xl text-[12px] max-w-[85%] leading-relaxed",
                                    msg.role === 'ai' ? "bg-blue-50/50 text-gray-800" : "bg-gray-900 text-white shadow-sm"
                                )}>
                                    {msg.content}
                                </div>
                            </div>
                        ...
                        {isThinking && (
                            <div className="flex gap-3">
                                ...
                                <div className="p-3 rounded-2xl bg-blue-50/50 text-gray-400 text-[12px] flex items-center gap-1">
            ...
            <div className="p-4 bg-gray-50/50 border-t border-gray-100">
                <div className="relative flex items-center bg-white border border-gray-100 rounded-2xl p-1.5 focus-within:shadow-md transition-all shadow-sm">
                    ...
                    <div className="flex items-center gap-1">
                        ...
                        <button
                            onClick={handleSend}
                            className="p-2.5 bg-blue-600 text-white rounded-xl hover:bg-blue-700 transition-all active:scale-95 shadow-md shadow-blue-100"
                        >
